...
const listings = [
    ...
    },
    {
        ...
        buildingSummary: {
            bedrooms: "4",
            bathrooms: "6",
            buildingFeatures: ["Detached", "Wood Frame", "Basement partially finished"],
            cooling: "Unknown",
            heating: "Forced Air",
            sewer: "Municipal",
            water: "Municipal",
...
const realtorFees = (listing) => {
    const listingPrice = listing.price.replace("$", "").replace(",", "");
    return listingPrice <= 450000 ? listingPrice * 0.025 : listingPrice * 0.02;
};
...
const listingAscendingly = [...listings].sort(
    (listingA, listingB) =>
        listingA.price.replace("$", "").replace(",", "") - listingB.price.replace("$", "").replace(",", "")
);
 ...
 */
//WRITE YOUR CODE BELOW
const newBuiltListings = listings.filter((listing) => listing.propertySummary.builtIn >= 1990);
