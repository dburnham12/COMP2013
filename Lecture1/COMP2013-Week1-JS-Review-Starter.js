...
for (let i = 0; i < keys.length; i++) console.log(student0[keys[i]]);
...
// console.log(id, studentName, dateOfBirth);
...
// if they have COMP205 in their courses then we will say they have the correct course load
// else we need to add COMP205 to their courseload and put a message out saying we added COMP205
// let courseLoadChecker = student0.courses.includes("COMP205")
//     ? "They have the correct course load"
//     : (student0.courses.push("COMP205"), console.log("We added COMP205"));

// console.log(courseLoadChecker);
...
const numArrayMultiple5 = [];

for (let i = 0; i < numArray.length; i++) {
    numArrayMultiple5.push(numArray[i] * 5);
}

console.log(numArrayMultiple5);
 ...
 */
const dataWith2002 = data.filter((stud) => stud.dateOfBirth.includes("2002")).map((stud) => stud.studentName);
console.log(dataWith2002);

const dataWithTuitionMoreThan12000 = data.filter((stud) => stud.tuitionPaid > 12000).map((stud) => stud.studentName);
 ...
 */
const dataSortedByTuitionPaid = data.sort((studB, studA) => studA.tuitionPaid - studB.tuitionPaid);
...
console.log(newData.map((stud) => stud.studentName));

//Remove a student object using filter method
const dataFiltered = newData.filter((stud) => !stud.studentName.includes("John Doe"));
console.log(dataFiltered.map((stud) => stud.studentName));
