...
server.post("/contacts", async (request, response) => {
    ...
    try {
        ...
        response.status(200).send({ message: `Contact is added successfully`, date: new Date(Date.now()) });
    } catch (error) {
...
server.delete("/contacts/:id", async (request, response) => {
    ...
    try {
        ...
        response.send({ message: `Contact is deleted with id ${id}`, date: new Date(Date.now()) });
    } catch (error) {
        response.status(400).send({ message: error.message });
    }
});
...
server.patch("/contacts/:id", async (request, response) => {
    ...
    try {
        ...
        response.send({ message: `Contact has been updated with id ${id}`, date: new Date(Date.now()) });
    } catch (error) {
        response.status(500).send({ message: error.message });
    }
});
